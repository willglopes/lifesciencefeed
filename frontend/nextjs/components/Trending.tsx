    ...
    async function loadArticles() {
      try {
        ...
        const items = entries.map((entry: any) => {
          ...
          if (catData?.attributes?.slug) {
            categorySlug = catData.attributes.slug;
          ...
          // category name
          let categoryName = '';
          const catDataName = raw.category?.data ?? raw.category;
          if (catDataName?.attributes?.name) {
            categoryName = catDataName.attributes.name;
          } else if (catDataName?.name) {
            categoryName = catDataName.name;
          }
          ...
          return { slug, categorySlug, categoryName, title, excerpt, imageUrl };
        });
  ...
  return (
    ...
    <section className="space-y-8 mb-12 pt-4">
      <div className='w-full pb-4 mb-6 grid grid-cols-1 sm:grid-cols-2 items-center inline-flex '>
        <span className='text-2xl font-serif'>Trending</span>
        <span className='text-md font-serif float-left'>&nbsp;on Life Science Feed</span>
          ...
          {primaryItems.map((item) => (
            <div key={item.slug} className="flex flex-col">
              <img
                ...
                alt={item.title}
                className="w-full h-40 object-cover rounded shadow-gray-400 shadow-sm hover:shadow-md transition-shadow duration-300"
              />
